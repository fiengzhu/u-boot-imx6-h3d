...
int board_mmc_init(bd_t *bis)
{
	...
	if (1){//hb_cuboxi_ == 2 || ver_15_ == 1) { /* HummingBoard 2 */
#if defined(CONFIG_MX6QDL)
...
static struct fb_videomode const lvds = {
        .name           = "LVDS",
        .refresh        = 60,
        .xres           = 320,
        .yres           = 480,
...
int board_video_skip(void)
{
  /*  int ret;
	...
	printf("Trying to initilize lvds...\n");

	int ret = ipuv3_fb_init(&lvds, 0, IPU_PIX_FMT_RGB666);
...
static void setup_display(void)
{
    ...
    // ============================================
    // Yuefeng Zhu modification add start, 170709
  	// second attemp
	...
	/* Turn on IPU clock */
	writel(0x201, IOMUXC_BASE_ADDR + 0x8); //IOMUXC_GPR2
	...
	// set LDB0 clk select to 000 (pll5) 
 	reg = readl(&mxc_ccm->cs2cdr);  
  	...
  	//
  	// Yuefeng Zhu modification add end, 170709
  	// ============================================

	// ============================================
    // Yuefeng Zhu modification delete start, 170709
    //
	// set video pll to 455MHz (24MHz * (38+6/12) / 4/4) = 8.25*7 MHz
	// somehow, the system will think the main clk to be 8.25 MHz, we have to match
	...
	writel(BF_ANADIG_PLL_VIDEO_NUM_A(6), &mxc_ccm->analog_pll_video_num);
	writel(BF_ANADIG_PLL_VIDEO_DENOM_B(12), &mxc_ccm->analog_pll_video_denom);

	writel(BF_ANADIG_PLL_VIDEO_DENOM_RSVD0(3), &mxc_ccm->ana_misc2);
	...
	writel(reg, &mxc_ccm->cs2cdr); */
	//
	// Yuefeng Zhu modification delete end, 170709
	// ============================================
