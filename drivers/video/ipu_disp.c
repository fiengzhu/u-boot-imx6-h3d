			    ...
			    uint32_t v_to_h_sync, ipu_di_signal_cfg_t sig)
{
	...
	if (sig.ext_clk) {
		if (!(g_di1_tvout && (disp == 1))) { /*not round div for tvout*/
			...
			if ((clk_get_usecount(g_pixel_clk[0]) == 0) &&
				...
				int di_parent_rate = clk_get_rate(di_parent);

				div  = clk_get_rate(di_parent) / rounded_pixel_clk;
				if (div % 2)
					div++;

				printf("div second is %d\n", div);
	...
	rounded_pixel_clk = clk_round_rate(g_pixel_clk[disp], pixel_clk);
	int ret = clk_set_rate(g_pixel_clk[disp], rounded_pixel_clk);
	if (ret < 0) {
		printf("set ldb di%d clk %d fail:%d\n", disp, rounded_pixel_clk, ret);
	}
	udelay(5000);

	//ret = clk_prepare_enable(g_pixel_clk[disp]);
	//if (ret < 0) {
	//	printf("enable ldb%d parent fail:%d\n", disp, ret);
	//}

	/* Get integer portion of divider */
	unsigned long g_para_clk = clk_get_parent(g_pixel_clk[disp]);
	div = clk_get_rate(clk_get_parent(g_pixel_clk[disp])) /
		rounded_pixel_clk;
	printf("round di%d clk = %dHz, pararent = %lu, div = %d\n", 
		disp, rounded_pixel_clk, g_para_clk, div);
